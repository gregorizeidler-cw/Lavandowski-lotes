...
from gpt_utils import get_chatgpt_response, process_large_transactions
import json
import decimal
import logging
import os
...
def get_gpt_analysis(prompt: str) -> str:
  ...
  from gpt_utils import process_large_transactions
  
  # Check for indicators that suggest a large number of transactions in the prompt
  ...
  # First try with process_large_transactions to handle potential large transaction sets
  try:
    result = process_large_transactions(prompt)
    
    # If any of the error indicators are found in the result, traditional processing failed
    # and we should keep the chunked processing result
    if any(indicator.lower() in result.lower() for indicator in large_dataset_indicators):
      return result  # Return the chunked processing result
    else:
      # If no error indicators, check if it returned a proper analysis with a risk score
      if "Risco de Lavagem de Dinheiro:" in result or "Score:" in result:
        return result  # Return the successful chunked processing result
      else:
        # Try regular processing as a fallback
        regular_result = get_chatgpt_response(prompt)
        return regular_result
  except Exception as e:
    # If chunked processing fails, fall back to regular processing
    regular_result = get_chatgpt_response(prompt)
    return regular_result
